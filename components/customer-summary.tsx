...
export function CustomerSummary({ customer, project }: CustomerSummaryProps) {
  ...
  // Mock health score calculation for demo
  const calculatedHealthScore = customer.healthScore

  // Mock health score pillars for demo
  const healthScorePillars = [
    { name: "Service Delivery", score: 85, trend: "improving", reasoning: "Response times improved by 15% this month" },
    { name: "Relationship Health", score: 78, trend: "stable", reasoning: "Regular engagement with key stakeholders maintained" },
    { name: "Growth Potential", score: 92, trend: "improving", reasoning: "Multiple expansion opportunities identified" },
    { name: "Risk Factors", score: 71, trend: "declining", reasoning: "Budget pressures and staff turnover concerns" }
  ]

  const getScoreColor = (score: number, trend: string) => {
    const baseColor = score >= 80 ? "bg-green-50 text-green-700" : score >= 60 ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"
    return baseColor
  }

  const getTrendIcon = (trend: string) => {
    switch (trend) {
      case "improving": return <TrendingUp className="h-3 w-3 text-emerald-600" />
      case "declining": return <TrendingDown className="h-3 w-3 text-rose-600" />
      default: return <Minus className="h-3 w-3 text-slate-600" />
    }
  }
  const healthColor =
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                >
                  ...
                  {showBreakdown && typeof document !== "undefined" && (
                    <div
