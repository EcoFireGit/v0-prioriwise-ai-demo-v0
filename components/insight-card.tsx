...
export function InsightCard({ insight, onClick }: InsightCardProps) {
  ...
  // Calculate opportunity values for Sales persona
  const monthlyOpportunity = insight.persona === "Sales" && insight.data["Monthly Revenue Gap"] 
    ? parseFloat(insight.data["Monthly Revenue Gap"].toString().replace(/[$,]/g, ''))
    : insight.persona === "Sales" && insight.data["Gap Value"] 
    ? parseFloat(insight.data["Gap Value"].toString().replace(/[$,\/yr]/g, '')) / 12
    : null

  const showOpportunityPill = monthlyOpportunity && monthlyOpportunity > 0
